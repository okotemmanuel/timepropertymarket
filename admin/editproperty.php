...
$cat=$cat2=$title=$amount=$details=$loc="";
 ...
 $sqls="SELECT * FROM property WHERE id='$idt'";
...
if(empty(getimagesize($_FILES["fileToUpload"]["tmp_name"]))){
	...
	$stmt = $conn->prepare("UPDATE property SET title=?,salerent=?,cat2=?,price=?,details=?,location=?,loc=?,img1=? WHERE id = ?");
$stmt->bind_param("sssssssss",$title,$cat,$cat2,$amount,$details,$location,$loc,$target_file,$idt);
...
/*/ prepare and bind
$stmt = $conn->prepare("INSERT INTO property(title,salerent,cat2,price,details,location,loc,img1,img2,img3,img4) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
$stmt->bind_param("sssssssssss",$title,$cat,$cat2,$amount,$details,$location,$loc,$target_file,$target_file2,$target_file3,$target_file4);
$stmt->execute();
...
$stmt->close();
 $conn->close();*/
	...
	header("location:addeimg.php?id='.$idt.'");
	exit;
...
<form method="POST"  enctype="multipart/form-data" AUTOCOMPLETE="off" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"> 
 ...
 <input type="hidden" name="val" value="<?php echo $idt?>"></input>
  <input type="hidden" name="img1" value="<?php echo $img1?>" ></input>

	<select value="<?php echo $cat?>" class="form-control" name="cat">
                <option value="RENT">RENT</option>
                <option value="SALE">SALE</option>
              </select>
			  
			  	<select value="<?php echo $cat2?>" name="cat2" class="form-control">
                <option value="RESINDETIALAPARTMENT">RESINDETIAL APARTMENT</option>
				 <option value="RESINDETIALBUNGALOW">RESINDETIAL BUNGALOW</option>
                <option value="COMMERCIALOFFICE">COMMERCIAL OFFICE</option>
				<option value="INDUSTRIALSHOPS">INDUSTRIAL SHOPS</option>
           ...
           <input name="title" value="<?php echo $title?>" type="text" class="form-control" placeholder="Title"><span class="error"> <?php echo $titleErr;?></span>
           <input value="<?php echo $amount?>" name="amount" type="text" class="form-control" placeholder="price/amount in (UGX)"><span class="error"> <?php echo $amountErr;?></span>
		  
<div>		  
<div>photo 1:    <input type="file"  name="fileToUpload" id="fileToUpload"><?php echo $img1?></input><span class="error"><?php echo $photErr;?></span>

 </div>
 
 
    	    <select name="loc" value="<?php echo $loc?>" class="form-control">
		   ...
		   <textarea name="details" rows="10" class="form-control" placeholder="Details"><?php echo $details?></textarea><span class="error"> <?php echo $detailsErr;?></span>
...
<div class="well">
  ...
  <style>
    ...
    <script>
	function fun(){
		window.location = "hop.php?value="+5;
	}
    function initMap() {
  ...
  var marker = new google.maps.Marker({
   ...
   document.getElementById('long').value = evt.latLng.lng().toFixed(3) ;
	...
	<input name="long" type="text" value="" id="long">
