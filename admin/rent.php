...
<div class="inside-banner">
  <div class="container"> 
    <span class="pull-right"><a href="index.php">Home</a> / for rent</span>
    <h2>FOR RENT</h2>
</div>
</div>
<!-- banner -->
...
<?php
	$sqls="SELECT * FROM property WHERE salerent='RENT' ORDER BY id DESC LIMIT 5";
...
if( empty($_POST['cat2']) && empty($_POST['loc']) && empty($_POST['low']) && empty($_POST['hig'])){
	...
	$sqls="SELECT * FROM property WHERE salerent='RENT' ORDER BY id DESC LIMIT 50";
        $results = mysqli_query($conn, $sqls);
...
$sqls="SELECT * FROM property WHERE WHERE salerent='RENT' AND  price BETWEEN $low AND $hig ORDER BY id DESC LIMIT 50";
